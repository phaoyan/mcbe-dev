...
// 左键检测
// 预注册：左键Dummy跟随行为
BehaviorUtils.register(entity_ids.dummy.lclick.lclick_dummy, (entity: Entity) => {
...
DPUtils.store().lclick_enable.register((target, curr, prev) => {
    ...
    const dummyTypes = [entity_ids.dummy.lclick.lclick_dummy]
    dummyTypes.forEach((dummyType)=>{
        ...
        if (curr && !dummy) {
            ...
            BehaviorUtils.bind(lclick.id, entity_ids.dummy.lclick.lclick_dummy)
        }
    })
