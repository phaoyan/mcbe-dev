...
world.afterEvents.dataDrivenEntityTrigger.subscribe(({ entity, eventId }) => {
    if (eventId !== EntityEventIds.Timer) return
...
// 监听Death
world.afterEvents.dataDrivenEntityTrigger.subscribe(({ entity, eventId }) => {
    if (eventId !== EntityEventIds.Death) return
    DPUtils.store().mob_dead.set(entity, true)
})

// 监听TargetAcquired
world.afterEvents.dataDrivenEntityTrigger.subscribe(({ entity, eventId }) => {
    if (eventId !== EntityEventIds.TargetAcquired) return
    DPUtils.store().mob_has_target.set(entity, true)
})

// 监听TargetEscape
world.afterEvents.dataDrivenEntityTrigger.subscribe(({ entity, eventId }) => {
    if (eventId !== EntityEventIds.TargetEscape) return
