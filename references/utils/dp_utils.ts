...
system.afterEvents.scriptEventReceive.subscribe(({ id, sourceEntity, message }) => {
    ...
    let target: Entity | World = sourceEntity
    if (message==="world") target = world
    if (message.includes(":")) {
    ...
    console.warn(`DP List: ${target.getDynamicPropertyIds().join(", ")}`)
    target.getDynamicPropertyIds().forEach(id => {
        console.warn(`${id}: ${JSON.stringify(target.getDynamicProperty(id))}`)
    })
})
